...
void InitEntities() {
	...
	guard.SetPosition({ 430,195 });
	guard.SetSize(45, 45);
	guard.InitSpriteSheet(0, 21, 4);
	

	//up move 33-39
	guard.SetSpriteClip(91, 226, 45, 45, 33);
	guard.SetSpriteClip(136, 226, 45, 45, 34);

	guard.SetAnchorOffset({ 0, 0 }, 33);
	guard.SetAnchorOffset({ 0, 0 }, 34);

	//left move 40-47
	guard.SetSpriteClip(271, 226, 45, 45, 40);
	guard.SetSpriteClip(315, 226, 45, 45, 41);
	
	guard.SetAnchorOffset({ 0, 0 }, 40);
	guard.SetAnchorOffset({ 0, 0 }, 41);

	//right move 48-54
	guard.SetSpriteClip(181, 226, 45, 45, 48);
	guard.SetSpriteClip(226, 226, 45, 45, 49);
	
	guard.SetAnchorOffset({ 0, 0 }, 48);
	guard.SetAnchorOffset({ 0, 0 }, 49);

	//down move 55-61
	guard.SetSpriteClip(0, 226, 45, 45, 55);
	guard.SetSpriteClip(46, 226, 45, 45, 56);
	
	guard.SetAnchorOffset({ 0, 0 }, 55);
	guard.SetAnchorOffset({ 1, 0 }, 56);

	//set guard waypoints
	guard.SetWaypointTargets({ {430,195},{-8,195},{-8,-10},{430,-10} });
	...
	//Setting grid Boundries
	gGrid.SetCellToBlocked(blueHouse2);
	gGrid.SetCellToBlocked(blueHouse3);
	gGrid.SetCellToBlocked(redHouse1);
	gGrid.SetCellToBlocked(redHouse2);

	//Test pathfinding
	PathFinder pathFinder;
	Int2 start(430, 195);
	Int2 end(158, 143);
	pathFinder.StartSearching(start, end);
	auto path = pathFinder.mPath;
	guard.SetPath
}
